...
When(
  `I create a new user "senior" with the following details 3:`,
...
Then(`I request the user with ID "3"`, async function () {
  const user = await findUserById(_context.userData.id);
  _context.createUserData = user;
});
Then(
  `I should receive a JSON response with the user's 3 details`,
  async function () {
...
Then(`The user's 3 category should be {string}`, async function (category) {

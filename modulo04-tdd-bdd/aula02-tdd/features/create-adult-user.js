...
When(
  `I create a new user "adult" with the following details:`,
...
Then(`I request the user with ID "2"`, async function () {
  const user = await findUserById(_context.userData.id);
  _context.createUserData = user;
});
Then(
  `I should receive a JSON response with the user's 2 details`,
  async function () {
...
Then(`The user's 2 category should be {string}`, async function (category) {
